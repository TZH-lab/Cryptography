...
| 签名公钥 `signature_public_key.pem` 文件大小（字节） |451字节 |
| 签名公钥指数 e 的值 |65537 |
| 签名公钥模数 N 的前  位十六进制 |  00:da:b3:58:03:8c:2b:5b|
| 加密私钥 `encryption_private_key.pem` 文件大小（字节） |1704b |
| 加密公钥 `encryption_public_key.pem` 文件大小（字节） |451b |
...
| 项目 | 你的结果 |
| :--- | :------- |
| 原始消息内容（你写入的文字） | 这是一条重要消息，需要签名保护。发送者：陶正洪，学号：2024010017|
| 密文文件 encrypted_message.bin 大小（字节） | 256|
| 密文 SHA-256 哈希值（完整的 64 位十六进 1bb52b187acde10f67d4df27bf715a05c800c4cb8e1514e5a765907d39fd23de
|
...
> 答：数字签名提供身份认证、完整性保护、不可否认性三项核心保证：
...
> 答：效率问题：RSA 等非对称算法运算速度远低于哈希函数。输入长度限制：RSA 单次运算只能处理不超过模数长度的数据块。标准化与通用性：哈希输出是固定长度的摘要，让签名算法的输入接口统一、简洁
...
> 答：RSA 签名的本质是对哈希值做模幂运算 ，运算结果的二进制长度不会超过模数 的长度。
...
> 答：哈希函数必须满足抗原像性、抗第二原像性、抗碰撞性三个性质，才能保障数字签名安全
...
> 答：对数字签名的影响：数字签名的安全性完全依赖哈希函数的抗碰撞性，SHA-1 碰撞漏洞会直接导致签名可被冒用。
...
> 答：签名密钥对：
签名私钥：用于生成数字签名（对消息哈希做私钥模幂运算），必须由持有者严格保密。
签名公钥：用于验证数字签名的合法性，可以公开分发。
加密密钥对：
加密私钥：用于解密密文，恢复明文，必须由接收方严格保密。
加密公钥：用于加密明文，生成密文，可以公开分发。
...
> 答：消息加密（公钥加密、私钥解密）解决的是机密性问题，数字签名（私钥签名、公钥验证）解决的是身份认证、完整性、不可否认性问题。
...
> 答：指消息的发送者无法事后抵赖自己发送过该消息、生成过对应签名，签名可以作为可信的第三方仲裁证据。
...
> 答：数字签名本身不能证明签名的生成时间：签名只绑定了消息内容和私钥持有者身份，不包含时间信息，签名者可以随意声称签名是任意时间生成的。解决方法是引入可信时间戳服务
...
> 答：使用 2048 位的原因：2048 位 RSA 在安全性和性能之间取得了最优平衡，是当前行业通用标准。它足以抵御现有算力和数论算法的攻击，满足绝大多数场景的长期安全需求；同时密钥生成、签名、加解密的计算开销适中，不会对系统性能造成过大负担。
1024 位已不安全：随着计算能力提升和整数分解算法进步，1024 位 RSA 已被证实可被国家级算力破解，NIST 等权威机构早已停止推荐在安全场景使用 1024 位密钥，存在模数被分解、私钥被破解的风险。
4096 位非必需，按需选择：4096 位 RSA 安全强度更高，但计算性能下降明显（密钥生成、签名、加解密速度远低于 2048 位，内存占用也更高）。普通业务场景下 2048 位已足够安全，4096 位性能代价过高；仅在高安全等级、需要超长期保密的场景（如核心根密钥、机密数据），才建议使用 4096 位来提升安全冗余。
...
> 答：后一个签名者对 “原始文件 + 前序签名” 整体进行签名，相当于为前序签名和原始文件做背书，证明自己认可文件内容和前序的签名行为，从而形成可追溯的信任链、审批链。
